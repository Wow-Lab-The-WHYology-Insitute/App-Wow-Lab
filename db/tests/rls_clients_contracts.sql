...
-- ============================================================================
begin;
  -- Setup (still privileged/unrestricted): one private_school client+contract,
  -- one corporate client+contract, both in wow-lab.
  ...
  select set_config('app.fixture_client_private', (
  ...
  select set_config('app.fixture_client_corporate', (
    insert into public.clients (organization_id, name, client_type, status)
    values (current_setting('app.test_org_wow_lab')::uuid, 'Fixture Corporate Client', 'corporate', 'active')
    returning id
  )::text, true);

  select set_config('app.fixture_contract_private', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status, billing_rule)
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client_private')::uuid, current_setting('app.test_legal_entity')::uuid, 'C1-TEST-PRIVATE-001', 'recurring_annual', 'signed', '95 lei/copil/sedinta')
    returning id
  )::text, true);
  select set_config('app.fixture_contract_corporate', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status, billing_rule)
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client_corporate')::uuid, current_setting('app.test_legal_entity')::uuid, 'C1-TEST-CORPORATE-001', 'one_off_event', 'signed', 'event contract')
    returning id
  )::text, true);
  ...
  select
    'finance_ops_a: sees the private_school contract' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_private')::uuid) as actual,
    1 as expected,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_private')::uuid) = 1 as pass;

  select
    'finance_ops_a: does NOT see the corporate contract (0 rows)' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_corporate')::uuid) as actual,
    0 as expected,
...
begin;
  ...
  select set_config('app.fixture_client_state', (
    insert into public.clients (organization_id, name, client_type, status)
  ...
  select set_config('app.fixture_contract_private', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status, billing_rule)
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client_private')::uuid, current_setting('app.test_legal_entity')::uuid, 'C1-TEST-PRIVATE-002', 'recurring_annual', 'signed', '80 lei/copil')
    returning id
  )::text, true);
  select set_config('app.fixture_contract_state', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status, billing_rule)
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client_state')::uuid, current_setting('app.test_legal_entity')::uuid, 'C1-TEST-STATE-001', 'framework', 'signed', 'grant framework contract')
    returning id
  )::text, true);
  ...
  select
    'finance_admin_a: sees the state_school contract' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_state')::uuid) as actual,
    1 as expected,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_state')::uuid) = 1 as pass;

  select
    'finance_admin_a: does NOT see the private_school contract (0 rows)' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_private')::uuid) as actual,
    0 as expected,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_private')::uuid) = 0 as pass;
rollback;
...
begin;
  ...
  select set_config('app.fixture_client', (
    insert into public.clients (organization_id, name, client_type, status)
    values (current_setting('app.test_org_wow_lab')::uuid, 'Fixture Masking Client', 'corporate', 'active')
    returning id
  )::text, true);
  select set_config('app.fixture_contract', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status, billing_rule)
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client')::uuid, current_setting('app.test_legal_entity')::uuid, 'C1-TEST-MASK-001', 'one_off_event', 'signed', 'SECRET-RATE-4200-lei')
    returning id
  )::text, true);
  ...
  select
    'catalina (ops): sees the client record (clients.read, no type restriction)' as check_name,
    (select count(*) from public.clients where id = current_setting('app.fixture_client')::uuid) as actual,
    1 as expected,
    (select count(*) from public.clients where id = current_setting('app.fixture_client')::uuid) = 1 as pass;

  select
    'catalina (ops): row still visible through contracts_billing_masked (row count = 1, not blocked)' as check_name,
    (select count(*) from public.contracts_billing_masked where id = current_setting('app.fixture_contract')::uuid) as actual,
    1 as expected,
    (select count(*) from public.contracts_billing_masked where id = current_setting('app.fixture_contract')::uuid) = 1 as pass;

  select
    'catalina (ops): billing_rule reads as NULL through contracts_billing_masked' as check_name,
    (select billing_rule from public.contracts_billing_masked where id = current_setting('app.fixture_contract')::uuid) as actual,
    null as expected,
    (select billing_rule from public.contracts_billing_masked where id = current_setting('app.fixture_contract')::uuid) is null as pass;
rollback;
...
begin;
  ...
  select set_config('app.fixture_contract', (
    ...
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client')::uuid, current_setting('app.test_legal_entity')::uuid, 'C1-TEST-SALES-NEG-001', 'one_off_event', 'draft')
    returning id
  )::text, true);
  ...
  select
    'sales_a: INSERT clients succeeds (clients.create)' as check_name,
  ...
  begin
    ...
    perform set_config('test.sales_insert_contract_blocked', v_blocked::text, true);
  end $$;

  select
    'sales_a: INSERT contracts is BLOCKED (no contracts.* write capability)' as check_name,
    current_setting('test.sales_insert_contract_blocked')::boolean as actual,
    true as expected,
    current_setting('test.sales_insert_contract_blocked')::boolean = true as pass;
  ...
  select
    'sales_a: UPDATE contracts (mark signed) affects 0 rows (no write capability)' as check_name,
...
begin;
  ...
  select
    'contract_admin_a: INSERT contracts succeeds' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract')::uuid) as actual,
    1 as expected,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract')::uuid) = 1 as pass;
  ...
  select
    'contract_admin_a: UPDATE contracts (mark signed) succeeds' as check_name,
    (select count(*) from updated) as actual,
    1 as expected,
    (select count(*) from updated) = 1 as pass;

  select
    'contract_admin_a: row_history captured the UPDATE with organization_id populated' as check_name,
    (
...
begin;
  ...
  select set_config('app.test_legal_entity_b', (
  ...
  select set_config('app.fixture_client_a', (
    insert into public.clients (organization_id, name, client_type, status)
    values (current_setting('app.test_org_wow_lab')::uuid, 'Fixture Org A Client', 'corporate', 'active')
    returning id
  )::text, true);
  select set_config('app.fixture_client_b', (
    insert into public.clients (organization_id, name, client_type, status)
    values (current_setting('app.test_org_wow_lab_test_b')::uuid, 'Fixture Org B Client', 'corporate', 'active')
    returning id
  )::text, true);

  select set_config('app.fixture_contract_a', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status)
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client_a')::uuid, current_setting('app.test_legal_entity_a')::uuid, 'C1-TEST-ORGA-001', 'one_off_event', 'signed')
    returning id
  )::text, true);
  select set_config('app.fixture_contract_b', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status)
    values (current_setting('app.test_org_wow_lab_test_b')::uuid, current_setting('app.fixture_client_b')::uuid, current_setting('app.test_legal_entity_b')::uuid, 'C1-TEST-ORGB-001', 'one_off_event', 'signed')
    returning id
  )::text, true);
  ...
  select
    'user_b: cannot see wow-lab''s (org A) fixture client (cross-org isolation)' as check_name,
    (select count(*) from public.clients where id = current_setting('app.fixture_client_a')::uuid) as actual,
    0 as expected,
    (select count(*) from public.clients where id = current_setting('app.fixture_client_a')::uuid) = 0 as pass;

  select
    'user_b: cannot see wow-lab''s (org A) fixture contract (cross-org isolation)' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_a')::uuid) as actual,
    0 as expected,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_a')::uuid) = 0 as pass;

  select
    'user_b: sees own org (org B) fixture client — isolation is scoped, not a blanket deny' as check_name,
    (select count(*) from public.clients where id = current_setting('app.fixture_client_b')::uuid) as actual,
    1 as expected,
    (select count(*) from public.clients where id = current_setting('app.fixture_client_b')::uuid) = 1 as pass;

  select
    'user_b: sees own org (org B) fixture contract — isolation is scoped, not a blanket deny' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_b')::uuid) as actual,
    1 as expected,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_b')::uuid) = 1 as pass;
rollback;
...
begin;
  ...
  select
    'owner_a: DELETE from clients is BLOCKED (deny-all, no grant anywhere — even for organization_owner)' as check_name,
...
begin;
  ...
  select set_config('app.fixture_contract_corporate', (
    insert into public.contracts (organization_id, client_id, legal_entity_id, contract_number, contract_type, status)
    values (current_setting('app.test_org_wow_lab')::uuid, current_setting('app.fixture_client_corporate')::uuid, current_setting('app.test_legal_entity')::uuid, 'C1-TEST-SABOTAGE-001', 'one_off_event', 'signed')
    returning id
  )::text, true);
  ...
  select
    'SABOTAGE: finance_ops_a sees the corporate contract, same assertion as Point 1, policy USING forced to (true)' as check_name,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_corporate')::uuid) as actual,
    0 as expected,
    (select count(*) from public.contracts where id = current_setting('app.fixture_contract_corporate')::uuid) = 0 as pass;
